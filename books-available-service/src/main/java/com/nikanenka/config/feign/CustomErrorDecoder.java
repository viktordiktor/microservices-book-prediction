...
import com.nikanenka.exceptions.BadRequestByBookAvailableException;
...
public class CustomErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
        try (InputStream bodyInputStream = response.body().asInputStream()) {
            ...
            if (exceptionResponse.getHttpStatus().equals(HttpStatus.NOT_FOUND)) {
                return new RequestNotFoundException(exceptionResponse.getMessage());
            }
            if (exceptionResponse.getHttpStatus().is4xxClientError()) {
                return new BadRequestByBookAvailableException(exceptionResponse.getMessage());
            }
